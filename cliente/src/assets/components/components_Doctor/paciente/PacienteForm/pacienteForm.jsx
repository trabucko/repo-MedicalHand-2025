...
const PacienteForm = ({
  ...
  const [vistaActual, setVistaActual] = useState("consultaActual");
  ...
  // Nuevo orden de tabs según lo solicitado
  const tabs = useMemo(
    () => [
  ...
  const renderConsultaActual = () => (
    <div className="px-content">
        ...
        <div className="px-form-grid">
          ...
          <div className="px-form-group px-full-width">
            ...
            <textarea
              ...
              onChange={(e) => handleInputChange("estiloVida", e.target.value)}
              rows="3"
            />
          </div>
        </div>
      ...
      <TabPanel value={activeTab} index={2}>
        ...
        <div className="px-card">
          ...
          <div className="px-exam-form">
            <div className="px-form-row">
              ...
              <button className="px-btn px-btn-primary" onClick={agregarExamen}>
                      ...
                      <div className="px-exam-info">
                        ...
                        <div className="px-exam-meta">
                          ...
                          <span className="px-exam-date">
                            {formatDateTime(examen.fechaSolicitud)}
                          </span>
                        </div>
                      </div>
        ...
        <div className="px-card">
          <h4 className="px-card-title">Cargar Resultados de Exámenes</h4>
          <div className="px-upload-form">
            <div className="px-form-group">
              <label className="px-label">Asociar resultado al examen:</label>
              <select
                className="px-select"
                value={pacienteData.examenSeleccionadoId || ""}
                onChange={(e) =>
                  handleInputChange("examenSeleccionadoId", e.target.value)
                }
              >
                <option value="">-- Selecciona un examen --</option>
                {pacienteData.examsRequested
                  .filter((ex) => ex.estado !== "completado")
                  .map((exam) => (
            ...
            <div className="px-form-group">
              <label className="px-label">Archivo:</label>
              <input
                type="file"
                className="px-input-file"
                onChange={handleFileChange}
                disabled={!pacienteData.examenSeleccionadoId}
              />
              {pacienteData.archivoParaSubir && (
                <div
                  className="px-file-selected-info"
                  style={{ marginTop: "8px", fontStyle: "italic" }}
                >
                  <span>
                    Archivo seleccionado: {pacienteData.archivoParaSubir.name}
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="px-card">
          <h4 className="px-card-title">Resultados de Exámenes Cargados</h4>
          <div className="px-results-list">
            {pacienteData.examsRequested.filter(
              (ex) => ex.resultados?.length > 0
            ).length === 0 ? (
              <div className="px-empty-state">
                <p className="px-empty-message">
              ...
              pacienteData.examsRequested.map(
                (exam) =>
                  exam.resultados?.length > 0 && (
                    <div key={exam.id} className="px-exam-result-group">
                      <h5 className="px-exam-result-title">{exam.nombre}</h5>
                      <div className="px-files-grid">
                        {exam.resultados.map((file, index) => (
                          <div key={index} className="px-file-card">
                            <div className="px-file-header">
                              <div className="px-file-icon">
                                {getFileIcon(file.type)}
                              </div>
                              <div className="px-file-info">
                                <h6 className="px-file-name">{file.name}</h6>
                                <span className="px-file-date">
                                  Subido: {formatDateTime(file.uploadedAt)}
                                </span>
                              </div>
                            </div>
                            <div className="px-file-actions">
                              ...
                              <button
                                className="px-btn-remove"
                                onClick={() =>
                                  eliminarExamenArchivo(exam.id, file.url)
                                }
                                title="Eliminar"
                              >
                                <FaTimes />
      ...
      <TabPanel value={activeTab} index={4}>
        ...
        <div className="px-medication-list">
          ...
          {!pacienteData.medicamentos?.length ? (
            <p className="px-empty-message">No se han agregado medicamentos</p>
          ) : (
              ...
              {pacienteData.notes.map((nota, i) => (
                <div key={nota.id || i} className="px-note-item">
                  <div className="px-note-header">
                    <span className={`px-note-badge ${nota.tipo || "default"}`}>
  ...
  return (
    <div className="px-main-container">
      ...
      {/* Vista Selector */}
      <div className="px-vista-selector">
        <div className="px-vista-tabs">
          <button
            className={`px-vista-tab ${
              vistaActual === "consultaActual" ? "px-vista-tab-active" : ""
            }`}
            onClick={() => setVistaActual("consultaActual")}
          >
            <FaCalendarAlt className="px-vista-tab-icon" />
            <span>Consulta Actual</span>
          </button>
          <button
            className={`px-vista-tab ${
              vistaActual === "consultasPrevias" ? "px-vista-tab-active" : ""
            }`}
            onClick={() => setVistaActual("consultasPrevias")}
          >
            <FaHistory className="px-vista-tab-icon" />
            <span>Consultas Previas</span>
          </button>
        </div>
      </div>

      {/* Tabs Navigation - Solo visible en Consulta Actual */}
      {vistaActual === "consultaActual" && (
            ...
            {tabs.map((tab, index) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="span_nav">{tab.label}</span>
                </button>
              );
            })}
          </div>
      ...
      {/* Content */}
      {vistaActual === "consultaActual" ? (
        renderConsultaActual()
      ) : (
        <ConsultasPrevias consultas={consultasPrevias} />
      )}
    </div>
  );
};
